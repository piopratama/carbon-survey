...
//     AdminApp.loadSamplingPoints(CURRENT_PROJECT_ID);
// };
...
AdminApp.togglePointIdVisibility = async function (isVisible) {
    window.AdminApp.showPointId = isVisible;
    AdminApp.showPointId = isVisible;

    AdminApp.samplingLayer.clearLayers();
    await AdminApp.loadSamplingPoints(CURRENT_PROJECT_ID);
};
...
AdminApp.samplingPopupHTML = function (p) {
    ...
    const progressBadge = `<span class="badge bg-info">${progress}</span>`;
